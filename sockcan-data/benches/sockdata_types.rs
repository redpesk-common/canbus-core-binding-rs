...
fn validate_can_bcmdata_invariants() {
    let opcode = sockcan::prelude::CanBcmOpCode::RxSetup;
    let frame = sockdata::types::CanBcmData::new(0x123, opcode, 42, vec![1, 2, 3, 4], 4);
...
fn bench_can_bcmdata_new_len8(c: &mut Criterion) {
    validate_can_bcmdata_invariants();

    c.bench_function("sockdata::CanBcmData::new (len=8, alloc)", |b| {
        b.iter(|| {
...
fn bench_can_bcmdata_serde_json_roundtrip(c: &mut Criterion) {
    validate_can_bcmdata_invariants();

    let opcode = sockcan::prelude::CanBcmOpCode::RxSetup;
    let frame =
        sockdata::types::CanBcmData::new(0x123, opcode, 42, vec![1u8, 2, 3, 4, 5, 6, 7, 8], 8);

    c.bench_function("sockdata::CanBcmData serde_json roundtrip", |b| {
        b.iter(|| {
            let json = serde_json::to_vec(black_box(&frame)).unwrap();
            let decoded: sockdata::types::CanBcmData =
...
criterion_group!(benches, bench_can_bcmdata_new_len8, bench_can_bcmdata_serde_json_roundtrip);
criterion_main!(benches);

#[cfg(test)]
mod tests {
    #[test]
    fn can_bcmdata_getters_are_consistent() {
        let opcode = sockcan::prelude::CanBcmOpCode::RxSetup;
        let frame = sockdata::types::CanBcmData::new(0x42, opcode, 7, vec![9, 8, 7], 3);
    ...
    #[test]
    fn can_bcmdata_json_roundtrip_is_stable() {
        let opcode = sockcan::prelude::CanBcmOpCode::RxSetup;
        let frame = sockdata::types::CanBcmData::new(0x123, opcode, 42, vec![1, 2, 3, 4], 4);

        let v1 = serde_json::to_value(&frame).unwrap();
        let decoded: sockdata::types::CanBcmData = serde_json::from_value(v1.clone()).unwrap();
