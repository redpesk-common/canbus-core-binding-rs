...
// Build script to generate Rust bindings from the Tesla Model 3 DBC file using `dbcparser`.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dbc_infile = "../samples/dbc_multiplexing/dbc/multiplexing.dbc";

    // generate parser outside of project git repo
    let dbc_outfile = "./src/__can-multiplexing.rs";
    ...
    DbcParser::new("DbcModel3")
        ...
        .range_check(true)
        .serde_json(true)
        .whitelist(vec![322]) // restrict generated code size to candump.log messages
        .generate()?;
    Ok(())
}
